...
- Add exact final-hand by-river runout-combination counts beside hand-ranking probabilities so each row's count and percentage describe that category only, not cumulative stronger hands.
...
- Add a deliberately simplified mobile learning layout optimized first for the iPhone 15 viewport (393 × 852 CSS pixels), focused on the table, current decision, and primary action controls. Hide the desktop top-level menu while playing. Show only the current hand-ranking summary and top strategy recommendation initially; reveal the full ranking reference, probabilities, ranges, and mixed strategy in accessible tap-to-expand sheets or accordions. Preserve comfortable touch targets and avoid shrinking the desktop information density into a small viewport.
...
7. Ship the first responsive mobile Play layout: table and actions first, compact current-hand and strategy summaries, tap-to-expand detail panels, and phone-sized visual QA. Leave Leduc evaluation, Subgame Solver table visualization, Human Training, bot-versus-bot play, multiplayer, and remaining expansion work on the roadmap until the active hold'em priority is complete.
