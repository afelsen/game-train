...
## Golden and independent checks

`golden/solver-results-v1.json` freezes configuration hashes, root action mixes, exploitability, and both seat EVs for the three curated spots. Run `.venv/bin/python scripts/verify_solver_goldens.py` to execute fresh native solves and compare them within explicit tolerances. The verifier also enforces probability normalization and that the two seat EVs sum to the starting pot. These are reproducibility goldens generated by the same implementation, not independent evidence.

An initial independent smoke check was performed against `bupticybee/TexasSolver` console revision `6dfb65b4d7ed081da509e8d8c3d82138c4708267`, compiled natively on Apple Silicon. To keep the game trees equivalent across engines, the comparison used an all-in-only turn tree with TexasSolver's documented sample ranges. The blocker-weighted root strategies differed by at most 2.47 percentage points, inside the recorded 3-point exploratory smoke threshold. The exact result and limitations are recorded in `golden/independent-crosscheck-v1.json`.

This is enough to catch integration regressions and establish that the engines broadly agree on a minimal tree. It is not enough to grade users by EV loss: the richer curated trees still require translated-tree comparison and per-combo EV agreement.
