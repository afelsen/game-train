...
function decodeFloat32(encoded: EncodedArray): Tensor {
  ...
  if (bytes.byteLength % 4 !== 0)
    throw new Error('Invalid Fullhouse tensor byte length');
...
async function loadModel() {
  if (!modelPromise) {
    ...
    modelPromise = fetch(new URL('../models/fullhouse-v17.json', baseUri))
      .then(async (response) => {
        if (!response.ok)
          throw new Error(
            `Fullhouse checkpoint failed to load (${response.status})`,
          );
        return response.json() as Promise<FullhouseArtifact>;
      })
      .then((artifact) => {
        if (
          artifact.schemaVersion !== 'game-train-fullhouse/v1' ||
          artifact.modelId !== FULLHOUSE_PROVIDER_ID
        ) {
...
export function decodeFullhouseArtifact(
  artifact: FullhouseArtifact,
): LoadedFullhouseModel {
  return {
    arrays: Object.fromEntries(
      Object.entries(artifact.arrays).map(([name, value]) => [
        name,
        decodeFloat32(value),
      ]),
    ),
...
const cardIndex = (card: string) =>
  SUITS.indexOf(card[1]) * 13 + RANKS.indexOf(card[0]);
...
function straightOutRanks(present: boolean[]) {
  ...
  for (let start = 0; start < 9; start += 1) {
    ...
    for (let rank = start; rank < start + 5; rank += 1)
      if (!present[rank]) missing.push(rank);
...
function drawFeatures(holes: number[], board: number[], street: number) {
  ...
  const boardFlushRanks = board
    .filter((card) => Math.floor(card / 13) === flushSuit)
    .map((card) => card % 13);
  const heroFlushRanks = holes
    .filter((card) => Math.floor(card / 13) === flushSuit)
    .map((card) => card % 13);
  const nutFlushDraw =
    flushDraw &&
    heroFlushRanks.length > 0 &&
    Math.max(...heroFlushRanks) ===
      Math.max(...boardFlushRanks, ...heroFlushRanks);
  const present = Array(13).fill(false) as boolean[];
  cards.forEach((card) => {
    present[card % 13] = true;
  });
  const straightRanks = straightOutRanks(present);
  const overlap =
    flushDraw && straightRanks > 0 ? Math.min(straightRanks, 2) : 0;
  const outs = (flushDraw ? 9 : 0) + straightRanks * 4 - overlap;
  return [
    Number(flushDraw),
    Number(straightRanks >= 2),
    Number(straightRanks === 1),
    outs / 20,
    Number(nutFlushDraw),
  ];
}
...
function textureFeatures(board: number[], heroHigh: number) {
  ...
  for (let start = 0; start < 9; start += 1) {
    if (
      [0, 1, 2, 3, 4].filter((offset) => present.has(start + offset)).length >=
      3
    )
      straightPossible = true;
  }
  if ([12, 0, 1, 2, 3].filter((rank) => present.has(rank)).length >= 3)
    straightPossible = true;
  return [
    Number(paired),
    Number(monotone),
    ranks.filter((rank) => rank > heroHigh).length / 5,
    pairs ? close / pairs : 0,
    Number(straightPossible),
  ];
}

function equityAt(table: Tensor, row: number, column = 0) {
  return table.data[
    table.shape.length === 1 ? row : row * table.shape[1] + column
  ];
}

export function encodeFullhouseFeatures(
  hand: BrowserPokerHand,
  model: LoadedFullhouseModel,
) {
  if (hand.toAct === null || hand.terminal)
    throw new Error('Fullhouse requires an active decision');
  ...
  const street = { preflop: 0, flop: 1, turn: 2, river: 3 }[
    hand.street as 'preflop' | 'flop' | 'turn' | 'river'
  ];
  ...
  if (street > 0)
    features[1] =
      eval7CompatibleScore([...(actor.holeCards ?? []), ...hand.board]) /
      MAX_EVAL7_SCORE;
  drawFeatures(holes, board, street).forEach((value, index) => {
    features[2 + index] = value;
  });
  features[7] = high / 12;
  features[8] = low / 12;
  ...
  textureFeatures(board, high).forEach((value, index) => {
    features[12 + index] = value;
  });
  features[17 + street] = 1;
  features[
    21 +
      Math.min(
        (actor.seat - hand.button + hand.playerCount) % hand.playerCount,
        5,
      )
  ] = 1;
  ...
  features[29] =
    Math.min(Math.max(minimumStack / Math.max(hand.pot, 1), 0), 10) / 10;
  ...
  hand.seats
    ...
    .slice(0, 5)
    .forEach((stack, index) => {
      features[31 + index] = stack / STARTING_STACK;
    });
  ...
  const blindOnly =
    hand.street === 'preflop' && hand.currentBet <= (hand.bigBlind ?? 100);
  features[36] = Math.min(raises.length, 4) / 4;
  features[37] = Number(
    lastRaiser !== null || (hand.currentBet > 0 && !blindOnly),
  );
  ...
  features[43] =
    Math.max(
      ...active
        .filter((seat) => seat.seat !== actor.seat)
        .map((seat) => seat.streetCommitted),
      0,
    ) / STARTING_STACK;
  features[44] =
    Math.max(
  ...
  legal.forEach((value, index) => {
    features[46 + index] = value;
  });
...
function abstractLegalMask(hand: BrowserPokerHand) {
  ...
  const raise = hand
    .legalActions()
    .find((action) => action.type === 'raise-to');
  if (actor.stack > owed && raise?.minAmount) {
    ...
    legal[3] = Number(
      potAfterCall >= hand.lastFullRaise && room > potAfterCall,
    );
    const opposingStacks = hand.seats
      .filter((seat) => seat.seat !== actor.seat && seat.status !== 'folded')
      .map((seat) => seat.stack);
    const effective = Math.min(
      actor.stack,
      Math.max(...opposingStacks, 0) || actor.stack,
    );
...
function linear(input: Float32Array, weight: Tensor, bias: Tensor) {
  ...
  for (let row = 0; row < output.length; row += 1) {
    ...
    for (let column = 0; column < width; column += 1)
      value += input[column] * weight.data[offset + column];
    output[row] = value;
  }
  return output;
}
...
function layerNorm(values: Float32Array, gamma: Tensor, beta: Tensor) {
  ...
  return Float32Array.from(
    values,
    (value, index) =>
      (gamma.data[index] * (value - mean)) / denominator + beta.data[index],
  );
}

export function inferFullhouseFeatures(
  features: Float32Array,
  legal: Float32Array,
  model: LoadedFullhouseModel,
) {
  const arrays = model.arrays;
  let hidden = leakyRelu(
    layerNorm(
      linear(features, arrays.trunk_w0, arrays.trunk_b0),
      arrays.trunk_ln0_g,
      arrays.trunk_ln0_b,
    ),
  );
  hidden = leakyRelu(
    layerNorm(
      linear(hidden, arrays.trunk_w1, arrays.trunk_b1),
      arrays.trunk_ln1_g,
      arrays.trunk_ln1_b,
    ),
  );
  ...
  const regrets = Float32Array.from(advantage, (item, index) =>
    legal[index] ? Math.max(value[0] + item - mean, 0) : 0,
  );
...
function mapAction(hand: BrowserPokerHand, name: (typeof ACTIONS)[number]) {
  ...
  if (name === 'fold')
    return legal.some((action) => action.type === 'fold')
      ? { type: 'fold', amount: null }
      : null;
  if (name === 'check-call') {
    if (legal.some((action) => action.type === 'check'))
      return { type: 'check', amount: null };
    if (legal.some((action) => action.type === 'call'))
      return { type: 'call', amount: null };
    return null;
  }
  if (name === 'all-in')
    return legal.some((action) => action.type === 'all-in')
      ? { type: 'all-in', amount: null }
      : null;
  const raise = legal.find((action) => action.type === 'raise-to');
  if (
    !raise ||
    raise.minAmount === null ||
    raise.maxAmount === null ||
    hand.toAct === null
  )
    return null;
  ...
  const target =
    owed === 0
      ...
      : hand.currentBet +
        Math.round((hand.pot + Math.min(owed, actor.stack)) * fraction);
  return {
    type: 'raise-to',
    amount: Math.max(raise.minAmount, Math.min(target, raise.maxAmount)),
  };
}

export async function fullhouseStrategy(
  hand: BrowserPokerHand,
): Promise<Strategy> {
  ...
  const mapped = modelActions.filter(
    (action) =>
      action.available && action.probability > 0 && action.legalAction,
  );
  ...
  const actions =
    total > 0
      ? mapped.map((action) => ({
          ...action,
          probability: action.probability / total,
        }))
      : [
          {
            abstractAction: 'check-call',
            probability: 1,
            available: true,
            legalAction: mapAction(hand, 'check-call'),
          },
        ];
  return {
